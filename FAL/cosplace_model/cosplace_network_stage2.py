...
def ensure_prompt_capacity(prompt_learner, label_tensor):
    """Dynamically enlarge prompt_learner.cls_ctx so that
       max(label) < prompt_learner.num_class.
       This only runs when we meet a brand-new class id.
    """
    max_id = int(label_tensor.max().item())
    if max_id < prompt_learner.num_class:
        return                                          # 已足够
    extra = max_id + 1 - prompt_learner.num_class       # 需补多少行
    device = prompt_learner.cls_ctx.device
    dtype  = prompt_learner.cls_ctx.dtype
    n_ctx  = prompt_learner.n_cls_ctx

    pad = torch.empty(extra, n_ctx, 512, dtype=dtype, device=device)
    nn.init.normal_(pad, std=0.02)

    # 拼到末尾并替换为新的 nn.Parameter
    new_cls_ctx = torch.cat([prompt_learner.cls_ctx, pad], dim=0)
    prompt_learner.cls_ctx = nn.Parameter(new_cls_ctx)
    prompt_learner.num_class = new_cls_ctx.shape[0]
# ---------------------------------------------------------------
