...
def resume_train(args: Namespace, output_folder: str, model: torch.nn.Module,
    ...
    ckpt_dp = next(iter(model_state_dict)).startswith("module.")
    cur_dp  = isinstance(model, torch.nn.DataParallel)
    if ckpt_dp and not cur_dp:             # 去掉 'module.'
        model_state_dict = {k.replace("module.", ""): v
                        for k, v in model_state_dict.items()}
    elif (not ckpt_dp) and cur_dp:         # 加上 'module.'
